    ...
    for (let i = 0; i <= maxRetries; i++) {
        try {
            ...
            if (i < maxRetries && shouldRetry(error)) {
                console.log(`Retrying after: ${error.name} (Attempt ${i + 1})`);
                continue; 
            }
            throw lastError;
        }
...
async function retryWithLogging(fn, maxRetries, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
        try {
            ...
            const delayToWait = Math.min(currentDelay, maxDelay);
            if (typeof onRetry === 'function') {
                onRetry(attempt, error, delayToWait);
            }
            await new Promise(resolve => setTimeout(resolve, delayToWait));
            currentDelay *= 2;
        }
    }
}
